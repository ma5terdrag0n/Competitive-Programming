...
int main(){
	...
	sort(all(v));
	vii pre(n+2, mp(0, inf)), suff(n+2, mp(0, inf));
	rep(n){
		pre[i+1].fr = max(pre[i].fr, v[i].fr);
		pre[i+1].sc = min(pre[i].sc, v[i].sc);
	}
	for(int i = n - 1 ; i >= 0 ; i--){
		suff[i+1].fr = max(suff[i+2].fr, v[i].fr);
		suff[i+1].sc = min(suff[i+2].sc, v[i].sc);
	}
	ll ans = 0;
	rep(n){
		ans = max(ans, min(pre[i].sc, suff[i+2].sc) - max(pre[i].fr, suff[i+2].fr));
	}
	cout << ans << endl;
}
